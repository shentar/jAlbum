...
public class FaceRecService implements FaceRecServiceInterface {
    ...
    private static class ServiceHolder {
        private static final FaceRecServiceInterface instance = new FaceRecService();
    }
    ...
    public void detectFaces(final FileInfo fi) {
    ...
    protected void detectOnePic(FileInfo fi) {
        try {
    ...
    public List<Face> getSortedFaces(long fid, int count, boolean needFileInfo) {
        if (fid >= 0) {
            ...
            if (count > 0) {
                ...
                int needCount = Math.min(count, flst.size());
    ...
    public List<Face> checkAndGetFaceidList() {
        ...
        for (long fid : faceids) {
            Face f = AppConfig.getInstance().getFacerServiceType() == 0 ?
                    FaceTableDao.getInstance().getNewestFaceByID(fid, true) :
                    FaceTableDao.getInstance().getBestFaceByID(fid, true);
            if (f != null) {
                fflist.add(f);
