...
public class GenerateHTML {
    ...
    public static String genIndexPage(List<?> flst, int rowCount, boolean needNavigate) {
        return genIndexPage(flst, rowCount, needNavigate, false);
    }

    public static String genIndexPage(List<?> flst, int rowCount, boolean needNavigate, boolean isFaceIDPage) {
        ...
        if (needNavigate) {
            indexPageNavi = genIndexNavigate(flst.get(0), flst.get(flst.size() - 1), isFaceIDPage);
    ...
    private static String genIndexNavigate(Object firstP, Object endP, boolean isFaceIDPage) {
        ...
        if (HeadUtils.isSuperLogin() && isFaceIDPage && firstP instanceof Face) {
            sb.append("<input id=\"deletefaces\" type=\"button\" value=\"屏蔽\">"
                    + separator);
            sb.append("<script type=\"text/javascript\" src=\"/js/jquery.alerts.js\"></script>"
                    + "<script type=\"text/javascript\">"
                    + "function changeUrl(url){window.history.pushState({},0,'//'+window.location.host+'/'+url);}"
                    + "function deletefaces(path){jConfirm('该操作将永久屏蔽次人物集，无法撤消，确认是否继续？','确认',function(r){if (r){"
                    + "changeUrl('/faces/');"
                    + "$.ajax({url:path,type:'DELETE',"
                    + "success:function(result){window.location.reload();}});}});}"
                    + "$(\"#deletefaces\").click(function(){deletefaces('/faces/" + ((Face) firstP).getFaceid() + "');});"
                    + "</script>");
        }
